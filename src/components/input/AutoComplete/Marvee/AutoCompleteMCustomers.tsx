...
export const AutoCompleteMCustomers = <TFieldValues extends FieldValues = FieldValues>({
  control,
  name,
  label = 'Cliente Marvee',
  description,
  ...
  const filters = useMemo(() => {
    ...
    // O endpoint /analysts/clientes/available aceita 'search' como parâmetro
    // que funciona tanto para nome quanto CPF/CNPJ
    return {
      search: query,
      page: 1,
      pageSize: 100,
  ...
  // Busca customers disponíveis (não cadastrados no sistema)
  ...
  const customers = useMemo(() => {
    ...
    // A lista já vem filtrada do backend (clientes não cadastrados)
    ...
    if (selectedCustomerId && selectedCustomerCache && selectedCustomerCache.id === selectedCustomerId) {
      const isInList = availableCustomers.some(c => c.id === selectedCustomerId);
      if (!isInList) {
  ...
  // Gera o label do cliente: CNPJ/CPF - FantasyName
